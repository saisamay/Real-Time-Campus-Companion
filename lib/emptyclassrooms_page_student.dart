...
class _ClassroomsPageState extends State<EmptyClassroomsPage> {
  ...
  // Show a read-only sheet with the current status (no actions)
  void _showOccupancySheet(BuildContext ctx, String roomName) {
    ...
    if (status == true) {
      ...
      message = 'Status unknown for this room.';
    }
      ...
      builder: (bCtx) {
        return Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            Center(child: _buildStatusPill(bCtx, roomName)),
            const SizedBox(height: 12),
            Padding(
              ...
              child: Text(
                ...
                style: TextStyle(fontSize: 14, color: Theme.of(bCtx).textTheme.bodyMedium?.color?.withOpacity(0.9)),
              ),
            ),
            const SizedBox(height: 16),
            Text(
              'This view is read-only. To report incorrect status, contact the administration.',
              textAlign: TextAlign.center,
              style: TextStyle(fontSize: 12, color: Theme.of(bCtx).textTheme.bodySmall?.color?.withOpacity(0.8)),
            ),
            const SizedBox(height: 18),
            FilledButton.tonal(
              onPressed: () => Navigator.pop(bCtx),
              child: const Padding(padding: EdgeInsets.symmetric(vertical: 12), child: Text('Close')),
            ),
          ]),
        );
      },
